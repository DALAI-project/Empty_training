...
from utils import load_binary_images_from_folder, initialize_model, calc_metrics
...
def train(args):
    ...
    #make dataloaders
    train_valid_paths = load_binary_images_from_folder(args.train_data)
    ...
    for epoch in range(args.epochs):
        ...
        for inputs, labels in tqdm(trainloader):
            ...
            #change outputs into binary classes. BINARY LOSS
            #outputs[outputs<0] = 0
            #outputs[outputs>0] = 1
            ...
            running_corrects += torch.sum(outputs.detach().squeeze() == labels.data)
        
        # Calculate model losses
        epoch_loss = running_loss / (len(trainloader)*args.batch)
        epoch_acc = running_corrects.double() / (len(trainloader)*args.batch)
        ...
        for inputs, labels in tqdm(validloader):
            ...
            with torch.no_grad():
                ...
                all_outs = np.append(all_outs, outputs.cpu().float().numpy()[:,1])
                all_labels = np.append(all_labels, labels.cpu().float().numpy())
        ...
        valmetrics = calc_metrics(all_outs, all_labels)
        ...
        writer.add_scalar("Accuracy/auroc", valmetrics['auroc'], epoch)
        writer.add_scalar("Precision/empty", valmetrics['prec_empty'], epoch)
        writer.add_scalar("Precision/ok", valmetrics['prec_ok'], epoch)
        writer.add_scalar("Recall/empty", valmetrics['rec_empty'], epoch)
        writer.add_scalar("Recall/ok", valmetrics['rec_ok'], epoch)
        fitness = 0.75 * valmetrics['rec_ok'] + 0.25 * valmetrics['acc_bal']
